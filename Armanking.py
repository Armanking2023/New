...
def logo(n):
   ...
   \033[1;96mArgentina Win Ty Free Dicci ARMAN  FIRE ðŸ”¥ðŸ’—
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada ARMAN ðŸ’” cp")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada ARMAN ðŸ’” ok")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_ARMAN ():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] cek ARMAN ðŸ’š akun ok\n [{hh}2{P}] cek ARMAN ðŸ’” akun cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{P}] tidak ARMAN ðŸ’š ok")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('OK/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada ARMAN ðŸ’š ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		except:sys.exit(f" [{M}>{P}] tidak ARMAN ðŸ’” cp")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('CP/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada ARMAN ðŸ’” cp")
		print(kk+buka+P)
	else:sys.exit(f" [{M}>{P}] isi yang benar")
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='Lihat ARMAN ðŸ’” Selanjutnya').get('href')
		if(link):
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
